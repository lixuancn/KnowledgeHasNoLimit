...
func (d *diskQueue)exit(deleted bool)error{
	...
	if deleted{
		...
		d.logf(INFO, "DISKQUEUE(%s) close", d.name)
	}
	close(d.exitChan)
	<- d.exitSyncChan
	if d.readFile != nil{
...
func (d *diskQueue)checkTailCorruption(depth int64){
	...
	if d.readFileNum != d.writeFileNum || d.readPos != d.writePos{
		...
		}else if d.readPos > d.writePos{
			d.logf(ERROR, "DISKQUEUE(%s) readPos > writePos (%d > %d), corruption, skipping to next writeFileNum and resetting 0...", d.name, d.readFileNum, d.writeFileNum)
		}
		d.skipToNextRWFile()
		d.needSync = true
	}
