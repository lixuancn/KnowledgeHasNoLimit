...
func TestPriorityQueue(t *testing.T) {
	...
	test.Equal(t, pq.Len(), c+1)
	test.Equal(t, cap(pq), c*2)
	for i := 0; i < c+1; i++ {
		item := heap.Pop(&pq)
		test.Equal(t, item.(*Item).Value.(int), i)
	}
	test.Equal(t, cap(pq), c/4)
}
...
func TestUnsortedInsert(t *testing.T) {
	...
	test.Equal(t, pq.Len(), c)
	test.Equal(t, cap(pq), c)
	sort.Sort(sort.IntSlice(ints))
	for i := 0; i < c; i++ {
		item, _ := pq.PeekAndShift(int64(ints[len(ints)-1]))
		test.Equal(t, item.Priority, int64(ints[i]))
	}
}
...
func TestRemove(t *testing.T) {
	...
	for i := 0; i < (c - 10 - 1); i++ {
		item := heap.Pop(&pq)
		test.Equal(t, lastPriority < item.(*Item).Priority, true)
