...
import (
	...
	"lixuancn/changbacron/internal/modules/httpclient"
	"lixuancn/changbacron/internal/modules/logger"
	"lixuancn/changbacron/internal/modules/notify"
	rpcClient "lixuancn/changbacron/internal/modules/rpc/client"
	pb "lixuancn/changbacron/internal/modules/rpc/proto"
)
...
func (task Task) RemoveAndAdd(taskModel models.Task) {
	task.Remove(cron.EntryID(taskModel.EntryID))
	task.Add(taskModel)
}
...
func (task Task) Add(taskModel models.Task) {
	...
	err := goutil.PanicToError(func() {
		...
		if err != nil{
			logger.Error("添加任务到调度器方法失败#", err)
		}
		taskModel.EntryID = int(entryId)
	})
	if err != nil {
...
func (task Task) NextRunTime(taskModel models.Task) time.Time {
	...
	for _, item := range entries {
		if int(item.ID) == taskModel.EntryID {
			return item.Next
		}
	}
...
func (task Task) Remove(entryID int) {
	serviceCron.Remove(cron.EntryID(entryID))
}
